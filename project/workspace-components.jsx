...
// ============================================================
// WORKSPACE COMPONENTS — Urban Master Planner
// Shared to window for use in the main workspace file
// ============================================================
...
const Icon = ({ name, size = 16, color = 'currentColor', strokeWidth = 1.5 }) => {
  const icons = {
    ...
    directSelect: <><circle cx="5" cy="5" r="1.5" fill={color}/><circle cx="19" cy="5" r="1.5" fill={color}/><circle cx="5" cy="19" r="1.5" fill={color}/><circle cx="19" cy="19" r="1.5" fill={color}/><path d="M5 5h14v14H5z" fill="none" strokeLinecap="round"/></>,
    ...
    addNode: <><circle cx="12" cy="12" r="2" fill={color}/><path d="M12 2v4M12 18v4M2 12h4M18 12h4" strokeLinecap="round"/></>,
    deleteNode: <><circle cx="12" cy="12" r="2" fill={color}/><path d="M4 4l16 16M20 4L4 20" strokeLinecap="round"/></>,
...
const workspaceTooltipStyles = {
  ...
  kbd: { background: 'rgba(255,255,255,0.15)', borderRadius: 3, padding: '1px 4px', fontSize: 10, fontFamily: 'monospace' }
};
const Tooltip = ({ label, shortcut, children, placement = 'right' }) => {
  ...
  return (
    <div style={workspaceTooltipStyles.wrapper}
      onMouseEnter={() => { timerRef.current = setTimeout(() => setShow(true), 400); }}
      onMouseLeave={() => { clearTimeout(timerRef.current); setShow(false); }}>
      {children}
      {show && (
        <div style={{ ...workspaceTooltipStyles.tip, ...placements[placement] }}>
          {label}
          {shortcut && <span style={workspaceTooltipStyles.kbd}>{shortcut}</span>}
        </div>
      )}
    </div>
  );
...
Object.assign(window, { Icon, Tooltip });
